...
function ForecastTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="glass-card !p-3 !rounded-lg !border-white/10">
      <p className="text-xs text-[var(--color-text-secondary)] mb-1.5">{label}</p>
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <motion.div variants={itemVariants} className="glass-card metric-card blue p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-[var(--color-text-muted)]">Projected Revenue (Dec)</p>
          </div>
          <p className="text-xl font-bold text-[var(--color-text-primary)]">{formatCurrency(3100000)}</p>
        ...
        <motion.div variants={itemVariants} className="glass-card metric-card green p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-[var(--color-text-muted)]">Confidence Level</p>
          </div>
          <p className="text-xl font-bold text-[#06d6a0]">87%</p>
          <span className="text-xs text-[var(--color-text-secondary)]">Based on 12m data</span>
        </motion.div>
        <motion.div variants={itemVariants} className="glass-card metric-card amber p-4">
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle className="w-4 h-4 text-[#f59e0b]" />
            <p className="text-xs text-[var(--color-text-muted)]">Bankruptcy Risk</p>
          </div>
          <p className="text-xl font-bold text-[#10b981]">Low</p>
          <span className="text-xs text-[var(--color-text-secondary)]">2.3% probability</span>
        </motion.div>
        <motion.div variants={itemVariants} className="glass-card metric-card violet p-4">
          ...
          <span className="text-xs text-[var(--color-text-secondary)]">Month-over-month</span>
        </motion.div>
      </div>
      ...
      <motion.div variants={itemVariants} className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--color-text-muted)]">Prophet + LSTM Model · 87% Confidence</p>
            </div>
          </div>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <AreaChart data={mockForecastData}>
            ...
            <Area type="monotone" dataKey="lower" stroke="none" fill="var(--color-surface-900)" name="Lower Bound" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Expense Forecast */}
        <motion.div variants={itemVariants} className="glass-card p-5">
          <h3 className="text-sm font-semibold text-[var(--color-text-primary)] mb-1">Expense Forecast</h3>
          <p className="text-xs text-[var(--color-text-muted)] mb-4">Predicted monthly burn rate</p>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={expenseForecast}>
              ...
              <YAxis tick={{ fill: "var(--color-text-muted)", fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v) => formatCurrencyShort(v)} />
              <Tooltip content={<ForecastTooltip />} />
        ...
        {/* Savings Forecast */}
        <motion.div variants={itemVariants} className="glass-card p-5">
          <h3 className="text-sm font-semibold text-[var(--color-text-primary)] mb-1">Savings Projection</h3>
          <p className="text-xs text-[var(--color-text-muted)] mb-4">On track to exceed target</p>
          <ResponsiveContainer width="100%" height={220}>
